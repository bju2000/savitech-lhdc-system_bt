...
#define LOG_TAG "a2dp_vendor_lhdc_ll"
...
#include "a2dp_vendor_lhdc_ll_encoder.h"
#include "bt_utils.h"
...
static const tA2DP_LHDC_CIE a2dp_lhdc_default_config = {
    A2DP_LHDC_VENDOR_ID,                // vendorId
    A2DP_LHDC_LL_CODEC_ID,                 // codecId
    A2DP_LHDC_SAMPLING_FREQ_44100,      // sampleRate
    A2DP_LHDC_CHANNEL_MODE_STEREO,      // channelMode
    BTAV_A2DP_CODEC_BITS_PER_SAMPLE_16,  // bits_per_sample
    false
};

static const tA2DP_ENCODER_INTERFACE a2dp_encoder_interface_lhdc_ll = {
    a2dp_vendor_lhdc_ll_encoder_init,
    a2dp_vendor_lhdc_ll_encoder_cleanup,
    a2dp_vendor_lhdc_ll_feeding_reset,
    a2dp_vendor_lhdc_ll_feeding_flush,
    a2dp_vendor_lhdc_ll_get_encoder_interval_ms,
    a2dp_vendor_lhdc_ll_send_frames,
    a2dp_vendor_lhdc_ll_set_transmit_queue_length};

UNUSED_ATTR static tA2DP_STATUS A2DP_CodecInfoMatchesCapabilityLhdcLL(
...
static tA2DP_STATUS A2DP_BuildInfoLhdcLL(uint8_t media_type,
...
// status code.
static tA2DP_STATUS A2DP_ParseInfoLhdcLL(tA2DP_LHDC_CIE* p_ie,
  ...
  if (p_ie->vendorId != A2DP_LHDC_VENDOR_ID ||
      p_ie->codecId != A2DP_LHDC_LL_CODEC_ID) {
    return A2DP_WRONG_CODEC;
  }
...
bool A2DP_IsVendorSourceCodecValidLhdcLL(const uint8_t* p_codec_info) {
  tA2DP_LHDC_CIE cfg_cie;

  /* Use a liberal check when parsing the codec info */
  return (A2DP_ParseInfoLhdcLL(&cfg_cie, p_codec_info, false) == A2DP_SUCCESS) ||
         (A2DP_ParseInfoLhdcLL(&cfg_cie, p_codec_info, true) == A2DP_SUCCESS);
}

bool A2DP_IsVendorPeerSinkCodecValidLhdcLL(const uint8_t* p_codec_info) {
  tA2DP_LHDC_CIE cfg_cie;
...
// otherwise the corresponding A2DP error status code.
static tA2DP_STATUS A2DP_CodecInfoMatchesCapabilityLhdcLL(
    const tA2DP_LHDC_CIE* p_cap, const uint8_t* p_codec_info,
    bool is_capability) {
  tA2DP_STATUS status;
  tA2DP_LHDC_CIE cfg_cie;
  ...
  /* parse configuration */
  status = A2DP_ParseInfoLhdcLL(&cfg_cie, p_codec_info, is_capability);
...
bool A2DP_VendorUsesRtpHeaderLhdcLL(UNUSED_ATTR bool content_protection_enabled,
                                  UNUSED_ATTR const uint8_t* p_codec_info) {
  // TODO: Is this correct? The RTP header is always included?
  return true;
}

const char* A2DP_VendorCodecNameLhdcLL(UNUSED_ATTR const uint8_t* p_codec_info) {
  return "LHDC LL";
}

bool A2DP_VendorCodecTypeEqualsLhdcLL(const uint8_t* p_codec_info_a,
  ...
  tA2DP_STATUS a2dp_status =
      A2DP_ParseInfoLhdcLL(&lhdc_cie_a, p_codec_info_a, true);
  if (a2dp_status != A2DP_SUCCESS) {
    LOG_ERROR(LOG_TAG, "%s: cannot decode codec information: %d", __func__,
              a2dp_status);
    return false;
  }
  a2dp_status = A2DP_ParseInfoLhdcLL(&lhdc_cie_b, p_codec_info_b, true);
  if (a2dp_status != A2DP_SUCCESS) {
    LOG_ERROR(LOG_TAG, "%s: cannot decode codec information: %d", __func__,
              a2dp_status);
    return false;
...
bool A2DP_VendorCodecEqualsLhdcLL(const uint8_t* p_codec_info_a,
                                const uint8_t* p_codec_info_b) {
  tA2DP_LHDC_CIE lhdc_cie_a;
  tA2DP_LHDC_CIE lhdc_cie_b;
...
int A2DP_VendorGetTrackSampleRateLhdcLL(const uint8_t* p_codec_info) {
  tA2DP_LHDC_CIE lhdc_cie;

  // Check whether the codec info contains valid data
  tA2DP_STATUS a2dp_status = A2DP_ParseInfoLhdcLL(&lhdc_cie, p_codec_info, false);
  if (a2dp_status != A2DP_SUCCESS) {
    LOG_ERROR(LOG_TAG, "%s: cannot decode codec information: %d", __func__,
              a2dp_status);
    return -1;
...
int A2DP_VendorGetTrackBitsPerSampleLhdcLL(const uint8_t* p_codec_info) {
  tA2DP_LHDC_CIE lhdc_cie;
...
int A2DP_VendorGetTrackChannelCountLhdcLL(const uint8_t* p_codec_info) {
  tA2DP_LHDC_CIE lhdc_cie;
...
int A2DP_VendorGetChannelModeCodeLhdcLL(const uint8_t* p_codec_info) {
  tA2DP_LHDC_CIE lhdc_cie;
...
bool A2DP_VendorGetPacketTimestampLhdcLL(UNUSED_ATTR const uint8_t* p_codec_info,
...
bool A2DP_VendorGetChannelSeparationLL(const uint8_t* p_codec_info){
  tA2DP_LHDC_CIE lhdc_cie;
...
bool A2DP_VendorBuildCodecHeaderLhdcLL(UNUSED_ATTR const uint8_t* p_codec_info,
                                     BT_HDR* p_buf,
...
void A2DP_VendorDumpCodecInfoLhdcLL(const uint8_t* p_codec_info) {
  tA2DP_STATUS a2dp_status;
  tA2DP_LHDC_CIE lhdc_cie;

  LOG_DEBUG(LOG_TAG, "%s", __func__);

  a2dp_status = A2DP_ParseInfoLhdcLL(&lhdc_cie, p_codec_info, true);
  if (a2dp_status != A2DP_SUCCESS) {
...
const tA2DP_ENCODER_INTERFACE* A2DP_VendorGetEncoderInterfaceLhdcLL(
    const uint8_t* p_codec_info) {
  if (!A2DP_IsVendorSourceCodecValidLhdcLL(p_codec_info)) return NULL;

  return &a2dp_encoder_interface_lhdc_ll;
}

bool A2DP_VendorAdjustCodecLhdcLL(uint8_t* p_codec_info) {
  tA2DP_LHDC_CIE cfg_cie;

  // Nothing to do: just verify the codec info is valid
  if (A2DP_ParseInfoLhdcLL(&cfg_cie, p_codec_info, true) != A2DP_SUCCESS)
    return false;

  return true;
}

btav_a2dp_codec_index_t A2DP_VendorSourceCodecIndexLhdcLL(
    UNUSED_ATTR const uint8_t* p_codec_info) {
  return BTAV_A2DP_CODEC_INDEX_SOURCE_LHDC_LL;
}

const char* A2DP_VendorCodecIndexStrLhdcLL(void) { return "LHDC LL"; }

bool A2DP_VendorInitCodecConfigLhdcLL(tAVDT_CFG* p_cfg) {
  if (A2DP_BuildInfoLhdcLL(AVDT_MEDIA_TYPE_AUDIO, &a2dp_lhdc_caps,
...
A2dpCodecConfigLhdcLL::A2dpCodecConfigLhdcLL(
    btav_a2dp_codec_priority_t codec_priority)
    : A2dpCodecConfig(BTAV_A2DP_CODEC_INDEX_SOURCE_LHDC_LL, "LHDC LL",
                      codec_priority) {
...
bool A2dpCodecConfigLhdcLL::init() {
  if (!isValid()) return false;

  // Load the encoder
  if (!A2DP_VendorLoadEncoderLhdcLL()) {
...
bool A2dpCodecConfigLhdcLL::useRtpHeaderMarkerBit() const { return false; }
...
bool A2dpCodecConfigLhdcLL::setCodecConfig(const uint8_t* p_peer_codec_info,
                                         bool is_capability,
  ...
  tA2DP_STATUS status =
      A2DP_ParseInfoLhdcLL(&sink_info_cie, p_peer_codec_info, is_capability);
  if (status != A2DP_SUCCESS) {
  ...
  if (int ret = A2DP_BuildInfoLhdcLL(AVDT_MEDIA_TYPE_AUDIO, &result_config_cie,
                         p_result_codec_config) != A2DP_SUCCESS) {
    LOG_ERROR(LOG_TAG,"%s: A2DP_BuildInfoLhdcLL fail(0x%x)", __func__, ret);
    goto fail;
  }
  ...
  if (is_capability) {
    status = A2DP_BuildInfoLhdcLL(AVDT_MEDIA_TYPE_AUDIO, &sink_info_cie,
  ...
  status = A2DP_BuildInfoLhdcLL(AVDT_MEDIA_TYPE_AUDIO, &result_config_cie,
                              ota_codec_config_);
  CHECK(status == A2DP_SUCCESS);
  return true;
